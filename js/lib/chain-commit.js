import { walletState, getExpectedNetworks } from "./wallet-state.js";
...
export async function commitMessage({ chain, message }) {
  const commitText = String(message || "");
  if (!commitText.trim()) {
  ...
  if (chain === "ethereum") {
    ...
    if (actualChainId !== expected.ethChainId) {
      throw new Error(`Ethereum wallet is on ${actualChainId}. Switch to ${expected.ethName} for ${mode} mode.`);
    }
    ...
    return {
      ...
      txHash,
      message: commitText,
      explorerUrl: getExplorerUrl(chain, mode, txHash),
    };
  }
